...
void uart0_init(void){
	...
	*/
	...
	// MCTLW register;  
	EUSCI_A0->MCTLW &= ~(EUSCI_A_MCTLW_BRS_MASK | EUSCI_A_MCTLW_BRF_MASK | EUSCI_A_MCTLW_OS16);
	...
	// IE register;      
	EUSCI_A0->IE &= ~(EUSCI_A_IE_TXCPTIE | EUSCI_A_IE_STTIE | EUSCI_A_IE_TXIE | EUSCI_A_IE_RXIE);

}

void uart2_init(void){
	//Set the UART to RESET state (set bit0 of EUSCI_A2->CTLW0 register to '1')
	EUSCI_A2->CTLW0 |= BIT(0);
	...
	*/
	...
	// clear first and second modulation stage bit fields
	// MCTLW register;  
	EUSCI_A2->MCTLW &= ~(EUSCI_A_MCTLW_BRS_MASK | EUSCI_A_MCTLW_BRF_MASK | EUSCI_A_MCTLW_OS16);
	...
	// disable interrupts (transmit ready, start received, transmit empty, receive full)	
	// IE register;      
	EUSCI_A2->IE &= ~(EUSCI_A_IE_TXCPTIE | EUSCI_A_IE_STTIE | EUSCI_A_IE_TXIE | EUSCI_A_IE_RXIE);
}
