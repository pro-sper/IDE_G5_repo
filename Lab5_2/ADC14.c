...
// Which value should be 1, which should be zero and send it in one shot 
...
void ADC0_InitSWTriggerCh6(void)
{
	...
	// REF_A->CTL0
	while ((REF_A->CTL0&0x1000) == 1){};	// #PO_added
  ...
  // 0     ADC14SC    ADC14 start             0b = No start (yet)
	...
	// ADC14->CTL0
  	ADC14->CTL0 &= ~(BIT31) | ~(BIT30);    			  // #PO_added: predivide by 1   
	ADC14->CTL0 &= ~(BIT29) | ~(BIT28) | ~(BIT27);    // #PO_added: SHM source 
	ADC14->CTL0 |=  (BIT26);    					  // #PO_added: SHM pulse mode
	ADC14->CTL0 &= ~(BIT25);    					  // #PO_added: invert sample-and-hold
	ADC14->CTL0 &= ~(BIT24) | ~(BIT23) | ~(BIT22);    // #PO_added: clock-divider = /1
	ADC14->CTL0 |=  (BIT21);						  // #PO_added: clock source select   100b = SMCLK
	ADC14->CTL0 &= ~(BIT20) | ~(BIT19);    
	ADC14->CTL0 &= ~(BIT20) | ~(BIT19); 			  // #PO_added: mode select = single-single

	ADC14->CTL0 |=  ~(BIT15) | ~(BIT14) 			  // #PO_added: ADC14SHT1x = 0011b (sample-and-hold time 32 clocks)
	ADC14->CTL0 |=   (BIT13 | BIT12);  
	ADC14->CTL0 |=  ~(BIT11) | ~(BIT10) 			  // #PO_added: ADC14SHT0x = 0011b (sample-and-hold time 32 clocks)
	ADC14->CTL0 |=   (BIT9 | BIT8);  

	ADC14->CTL0 &=  ~(BIT7);                    	  // #PO_added: ADC14MSC = 0b (not multiple), 
	ADC14->CTL0 |=   (BIT4)							  // #PO_added: ADC14ON = 1b (powered up)

	// Set ADC14CONSEQx (bits 18-17) for single-channel, single-conversion
	ADC14->CTL0 &= ~(BIT18 | BIT17);

	// Set ADC14ENC (bit 1) to enable conversion
	ADC14->CTL0 |= BIT1;

	// Set ADC14SC (bit 0) to start conversion
	ADC14->CTL0 |= BIT0;
  ...
  // 1-0   ADC14PWRMD ADC power modes        00b = Regular power mode
	...
	// ADC14->CTL1
    ADC14->CTL1 &= ~(BIT20 | BIT19 | BIT18 | BIT17 | BIT16);    // #PO_added: start addr 00000b = ADC14MEM0
	ADC14->CTL1 |= (BIT5 | BIT4);		 					    // #PO_added: resolution 11b = 14 bit, 16 clocks
	ADC14->CTL1 &= ~(BIT3 | BIT2 | BIT1 | BIT0);    			// #PO_added: zeroes for 3210
  ...
  // 4-0  ADC14INCHx  Input channel        0110b = A6, P4.7
	...
	// ADC14->IER0
	// ADC14->IER1
  	ADC14->MCTL[0] &= ~(BIT15 | BIT14 | BIT13 | BIT12);		// #PO_added: zeroes at 15 downto 12
	ADC14->MCTL[0] &= ~(BIT11 | BIT10 | BIT9);				// #PO_added: zeroes at 11 downto 9
	ADC14->MCTL[0] |= (BIT8);								// #PO_added: one at bit 8
	ADC14->MCTL[0] |= (BIT7);								// #PO_added: one at bit 7 = End of sequence
	ADC14->MCTL[0] &= ~(BIT4 | BIT3);
	ADC14->MCTL[0] |= (BIT2 | BIT1);
	ADC14->MCTL[0] &= ~(BIT0);
